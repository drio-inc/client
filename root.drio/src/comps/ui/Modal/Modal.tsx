...
const Modal = ({ label, onClick, children, identifier }: IModalProps) => {
  const isOpen = useAppSelector(({ ui }) => ui.modalBoolObject[identifier]);
  ...
  return (
    <CustomModal.Root open={isOpen} onOpenChange={onClick}>
      ...
      <CustomModal.Portal>
        ...
        <CustomModal.Content className="data-[state=open]:animate-contentShow data-[state=closed]:animate-contentHide fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-white shadow-sm focus:outline-none z-[1001]">
